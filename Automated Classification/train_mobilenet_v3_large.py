...
#
# 训练命令示例： 
#   python train_mobilenet_v3_large_weight.py --model mobilenet_v3_large --num_classes 2
############################################################################################################
...
parser.add_argument('--model', type=str, default="mobilenet_v3_large", help='select a model for training')
parser.add_argument('--device', default='cuda', help='device id (i.e. 0 or 0,1 or cpu)')

parser.add_argument(
    '--weights',
    type=str,
    default=r"/model_pth/mobilenet_v3_large.pth",
...
def main(args):
    ...
    # ====================== MobileNetV3-Large 预训练权重加载 ======================
    if args.weights != "":
        assert os.path.exists(args.weights), \
            f"weights file: '{args.weights}' not exist."

        print(f"=> Loading MobileNetV3-Large pretrained weights from: {args.weights}")
        ...
        # 删除 MobileNetV3-Large 的最后一层分类层（避免 num_classes 不一致）
        # torchvision.mobilenet_v3_large 最后一层是 classifier[3]
        del_keys = ["classifier.3.weight", "classifier.3.bias"]
        for k in del_keys:
            if k in state_dict:
        ...
        print(f"Loaded {len(filtered_dict)}/{len(model_dict)} layers from MobileNetV3-Large pretrained weights.")
    else:
    ...
    log_txt = os.path.join(save_path, "mobilenet_v3_large_log.txt")
    weight_path = os.path.join(save_path, "mobilenet_v3_large.pth")
