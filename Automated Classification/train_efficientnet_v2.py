...
#
# 训练命令示例： 
#   python train_efficientnet_v2_l_weight.py --model efficient_v2_large --num_classes 2
############################################################################################################
...
parser.add_argument('--model', type=str, default="efficient_v2_large", help='select a model for training')
parser.add_argument('--device', default='cuda', help='device id (i.e. 0 or 0,1 or cpu)')

parser.add_argument(
    '--weights',
    type=str,
    default=r"/model_pth/efficientnet_v2_l.pth",
...
def main(args):
    ...
    # ====================== EfficientNetV2-L 预训练权重加载 ======================
    if args.weights != "":
        assert os.path.exists(args.weights), \
            f"weights file: '{args.weights}' not exist."

        print(f"=> Loading EfficientNetV2-L pretrained weights from: {args.weights}")
        ...
        # 删除 EfficientNetV2-L 的最后一层分类层（避免 num_classes 不一致）
        # torchvision.efficientnet_v2_l: classifier[1] 为最后的 Linear 层
        ...
        print(f"Loaded {len(filtered_dict)}/{len(model_dict)} layers from EfficientNetV2-L pretrained weights.")
    else:
    ...
    log_txt = os.path.join(save_path, "efficientnet_v2_l_log.txt")
    weight_path = os.path.join(save_path, "efficientnet_v2_l.pth")
