...
#
# 训练命令示例： 
#   python train_googlenet.py --model googlenet --num_classes 2
############################################################################################################
...
parser.add_argument('--model', type=str, default="googlenet", help='select a model for training')
parser.add_argument('--device', default='cuda', help='device id (i.e. 0 or 0,1 or cpu)')

parser.add_argument(
    '--weights',
    type=str,
    default=r"/model_pth/googlenet.pth",
...
def main(args):
    ...
    if args.weights != "":
        ...
        # 可以选择性删除分类头（如果名字对得上）
        del_keys = ["classifier.weight", "classifier.bias", "fc.weight", "fc.bias"]
        for k in del_keys:
            if k in state_dict:
    ...
    log_txt = os.path.join(save_path, "googlenet_log.txt")
    weight_path = os.path.join(save_path, "googlenet.pth")
