...
#
# 训练命令示例： 
#   python train_resnet101_weight.py --model resnet_big --num_classes 2
############################################################################################################
...
parser.add_argument('--model', type=str, default="resnet_big", help='select a model for training')
parser.add_argument('--device', default='cuda', help='device id (i.e. 0 or 0,1 or cpu)')

parser.add_argument(
    '--weights',
    type=str,
    default=r"/model_pth/resnet101.pth",
...
def main(args):
    ...
    # ====================== 更鲁棒的 ResNet101 预训练权重加载 ======================
    if args.weights != "":
        assert os.path.exists(args.weights), \
            "weights file: '{}' not exist.".format(args.weights)

        print(f"=> Loading ResNet101 pretrained weights from: {args.weights}")
        # ★ 关键修改：一律先加载到 CPU，避免 CPU / CUDA storage 冲突
        state_dict = torch.load(args.weights, map_location='cpu')

        # 兼容几种常见封装：{'state_dict': ...}, {'model': ...}, 直接 state_dict
        ...
        # 只加载形状完全匹配的层，避免 fc 层 num_classes 不一致出错
    ...
    log_txt = os.path.join(save_path, "resnet101_log.txt")
    weight_path = os.path.join(save_path, "resnet101.pth")
