...
#
# 训练命令示例： 
#   python train_repvit_m0_9_weight.py --model repvitm09 --num_classes 2
############################################################################################################
...
parser.add_argument('--model', type=str, default="repvitm09", help='select a model for training')
parser.add_argument('--device', default='cuda', help='device id (i.e. 0 or 0,1 or cpu)')

parser.add_argument(
    '--weights',
    type=str,
    default=r"/model_pth/repvit_m0_9_distill_450e.pth",
...
def main(args):
    ...
    if args.weights != "":
        ...
        print(f"=> Loading RepViT-M0.9 pretrained weights from: {args.weights}")
        ...
        # 只加载形状完全匹配的层（分类头维度不一致会自动跳过）
        for k, v in state_dict.items():
            if k in model_dict and v.shape == model_dict[k].shape:
                filtered_dict[k] = v

        # 打印一下加载统计信息，方便确认
        skipped_keys = [k for k in state_dict.keys() if k not in filtered_dict]
        print(f"Total keys in ckpt: {len(state_dict)}")
        print(f"Matched & loaded keys: {len(filtered_dict)}")
        print(f"Skipped keys (usually包含分类头等不匹配层): {len(skipped_keys)}")

        model_dict.update(filtered_dict)
        model.load_state_dict(model_dict)
        print("=> RepViT-M0.9 pretrained backbone loaded (shape-matching).")
    else:
    ...
    log_txt = os.path.join(save_path, "repvit_m0_9_log.txt")
    weight_path = os.path.join(save_path, "repvit_m0_9.pth")
