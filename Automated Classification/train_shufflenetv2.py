...
#
# 训练命令示例： 
#   python train_shufflenetv2_x1_weight.py --model shufflenet --num_classes 2
############################################################################################################
...
parser.add_argument('--model', type=str, default="shufflenet", help='select a model for training')
parser.add_argument('--device', default='cuda', help='device id (i.e. 0 or 0,1 or cpu)')

parser.add_argument(
    '--weights',
    type=str,
    default=r"/model_pth/shufflenetv2_x1.pth",
...
def main(args):
    ...
    # ====================== ShuffleNetV2 x1.0 预训练权重加载 ======================
    if args.weights != "":
        assert os.path.exists(args.weights), \
            f"weights file: '{args.weights}' not exist."

        print(f"=> Loading ShuffleNetV2 x1.0 pretrained weights from: {args.weights}")
        ...
        # 删除 ShuffleNetV2 x1.0 的最后一层分类层（避免 num_classes 不一致）
        # torchvision.shufflenet_v2_x1_0 的最后一层是 fc
        ...
        print(f"Loaded {len(filtered_dict)}/{len(model_dict)} layers from ShuffleNetV2 x1.0 pretrained weights.")
    else:
    ...
    log_txt = os.path.join(save_path, "shufflenetv2_x1_log.txt")
    weight_path = os.path.join(save_path, "shufflenetv2_x1.pth")
