...
parser.add_argument('--model', type=str, default="swin_transformer", help=' select a model for training') 
...
def main(args):
    ...
    # ====================== 使用固定路径的 Swin Base 预训练权重 ======================
    swin_weights_path = "/model_pth/swin_base_patch4_window7_224.pth"
    if os.path.exists(swin_weights_path):
        print(f"=> Loading Swin Base pretrained weights from: {swin_weights_path}")
        state_dict = torch.load(swin_weights_path, map_location=device)

        # 有些权重文件外面会包一层 'model' 或 'state_dict'，这里做下兼容
        ...
        # 只加载形状完全匹配的权重（避免分类头 num_classes 不一致导致错误）
        ...
        print(f"⚠ 找不到预训练权重文件：{swin_weights_path}，将使用随机初始化参数训练。")
    # ============================================================================
    ...
    for epoch in range(args.epochs):
        ...
        with open(os.path.join(save_path, "train_swin_transformer.txt"), 'a') as f: 
            f.writelines(
        ...
        if val_acc > best_acc:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(save_path, "train_swin_transformer.pth")
            ) 
