...
#
# 训练命令示例： 
#   python train_vgg16_weight.py --model vgg --num_classes 2
############################################################################################################
...
def main(args):
    ...
    if args.weights != "":
        ...
        print(f"=> Loading VGG16 pretrained weights from: {args.weights}")
        ...
        # 删除 VGG16 的最后一层分类层（避免 num_classes 不一致）
        # torchvision.vgg16 最后一层是 classifier[6]
        ...
        print(f"Loaded {len(filtered_dict)}/{len(model_dict)} layers from VGG16 pretrained weights.")
    else:
    ...
    log_txt = os.path.join(save_path, "vgg16_log.txt")
    weight_path = os.path.join(save_path, "vgg16.pth")
